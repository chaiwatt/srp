                    ...
                    <div class="smart-widget-body  padding-md">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    @php( $value1 = $allocation->where('section_id' , $item->section_id)->sum('allocation_price') )
                                    @php( $value2 = $transfer->where('section_id' , $item->section_id)->sum('transfer_price') )
                                    @php( $value3 = $payment->where('section_id' , $item->section_id)->sum('payment_salary') )
                                    @php( $value4 = $refund->where('section_id' , $item->section_id)->sum('refund_price') )
                                    <td style="vertical-align: middle;" rowspan="{{ count($position)+1 }}">{{ $item->sectionname }}</td>
                                    ...
                                    <tr>
                                        ...
                                        @php( $query = $employ->where('position_id' , $value->position_id)->where('section_id' , $item->section_id)->count() )
                                        <td class="text-center">{{ $query }}</td>
                                        @php( $query = $employ->where('position_id' , $value->position_id)->where('section_id' , $item->section_id)->where('generate_status' , 1)->count() )
                                        <td class="text-center">{{ $query }}</td>
                                    </tr>
                                    @endforeach
                                    @endif
		...
		Morris.Bar({
            ...
            data: [
                ...
                @foreach( $position as $item )
                    @php( $value = $generate->where('position_id' , $item->position_id)->count() )
                    @php( $query = $employ->where('position_id' , $item->position_id)->count() )
                    { position: '{{ $item->position_name }}', hire: {{ $value }} , employ : {{ $query}} },
                @endforeach
                @endif
            ],
            xkey: 'position',
            ykeys: ['hire' , 'employ'],
            labels: ['จัดจ้างแล้ว' , 'จำนวนจัดสรร'],
            resize: true,
            parseTime:false,
        });
	})
