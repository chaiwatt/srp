        ...
        <div class="col-sm-12">
            ...
            <div class="form-inline no-margin">
                ...
                @if( $month != null || $quater != null )
                    <a href="{{ URL::route('recurit.report.section.recurit.export.excel',['month' => $month , 'quater' => $quater ]) }}" class="btn btn-sm btn-info">Excel</a>
                    <a href="{{ URL::route('recurit.report.section.recurit.export.pdf',['month' => $month , 'quater' => $quater ]) }}" class="btn btn-sm btn-warning">PDF</a>
                @endif
                            ...
                            <tbody>
                                ...
                                @foreach( $employ as $key => $item )
                                    ...
                                    @if (count($_register) !=0 )
                                        ...
                                        @endphp
                                        <tr>
                                            <td>{{ $_register->prefixname }}</td>
                                            <td>{{ $_register->name }}</td>
                                            <td>{{ $_register->lastname }}</td>
                                            <td>{{ $_register->person_id }}</td>
                                            <td>@if (!Empty($_education)){{ $_education->register_education_name }} @endif</td>
                                            <td>{{ $_register->ageyear }}</td>
                                            <td>{{$_register->contract_no}}</td>
