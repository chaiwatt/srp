    ...
    <div class="row">
        <div class="col-sm-9">
            <div class="page-title">
                รายการฝึกอบรมวิชาชีพ ปีงบประมาณ : {{ $project->year_budget }}
            </div>
        </div>
        <div class="col-sm-3">
            <div class="pull-right">
                <a href="{{ url('occupation/project/department/create') }}" class="btn btn-success"><i class="fa fa-plus"></i> เพิ่มโครงการอบรม</a>
            </div>
        </div>
    </div>
</div>
                                    ...
                                    @if( count($readiness) > 0 )
                                        ...
                                        @endphp
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    @if ($item->project_status == 0)
                                                        <a href="{{ url('occupation/project/department/edit/'.$item->project_readiness_id) }}" class="btn btn-info btn-xs" >เพิ่มเติม</a>
                                                        <a href="{{ url('occupation/project/department/delete/'.$item->project_readiness_id) }}" class="btn btn-danger btn-xs" onclick="return confirm('ยืนยันการลบโครงการ')" >ลบ</a>
                                                    @else
                                                    <a href="{{ url('occupation/project/department/sectionlist/'.$item->project_readiness_id) }}" class="btn btn-info btn-xs" >รายละเอียด</a>
                                                    @endif
                                                </td>
                                            </tr>
