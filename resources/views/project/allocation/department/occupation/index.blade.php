...
<div class="padding-md">
    <ul class="breadcrumb">
        ...
        <li>โอนงบประมาณฝึกอบรมวิชาชีพ</li>    
    </ul>

    <div class="row">
        <div class="col-sm-9">
            <div class="page-title">
                โอนงบประมาณฝึกอบรมวิชาชีพ ปีงบประมาณ : {{ $project->year_budget }} 
            </div>
        </div>
    </div>
                    ...
                    <select class="form-control" name="readiness"  >
                        @if(count($projectreadiness) > 0)
                            @foreach( $projectreadiness as $item )
                                <option value="{{$item->project_readiness_id}}" >{{ $item->project_readiness_name }}</option>
                            @endforeach
                        @endif
                    </select>
            </div>
    ...
    @if( $search != "" )
        <div class="row">
            <div class="col-md-12">
                {!! Form::open([ 'url' => 'project/allocation/department/occupation/create' , 'method' => 'post' ]) !!} 
                <input type="hidden" name="readiness_id" value="{{$readiness}}">
                    <div class="smart-widget widget-dark-blue">

                        <div class="smart-widget-header"> งบประมาณฝึกอบรมวิชาชีพ คงเหลือ = {{ number_format($transfer - $payment + $refund,2) }} บาท</div>
                        {{-- <div class="smart-widget-header">  ( งบประมาณคงเหลือ ) </div> --}}
                            ...
                            <div class="smart-widget-body  padding-md">
                                ...
                                @endphp
                                    ...
                                    <label>{{ $value->sectionname }}</label> @if ($value->completed == 1) <span class="text-success">(จบโครงการ@if ($waitrefund > 0) <span class="text-danger"> เงินรอคืน {{$waitrefund}} บาท </span> @endif)</span> @endif
                                        ...
                                        <input type="number" name="number[{{$value->section_id}}]" class="form-control" step="0.01" autocomplete="off" value="{{ $value->budget }}" @if ($value->completed == 1)  @endif>
                                    </div>
