    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="page-title">
                รายการจัดสรรงบประมาณ ปีงบประมาณ {{$project->year_budget}}
            </div>
        </div>
    </div>
</div>
                            ...
                            @foreach( $budget as $key => $item )
                                <tr>
                                    ...
                                    @foreach( $department as $key1 => $value )
                                        ...
                                        <td><input type="number" class="form-control text-center" min="0.00" step="0.01" name="number[{{ $item->budget_id }}][{{  $value->department_id }}]" value="{{ $query }}" /></td>
                                    @endforeach
                                    @endif
                                    @php
                                         ...
                                         $sumrefund +=  $query ;
                                    @endphp
                                    <td class="text-right text-danger"> {{ number_format( $query , 2 ) }}</td>
                                </tr>
                            @endforeach
                            @endif
                                <tr>
                                    ...
                                    @foreach( $department as $key1 => $value )
                                        ...
                                        <td class="text-center"><strong> {{ number_format( $number , 2 ) }}</strong></td>
                                    @endforeach
                                    @endif
                                    <td class="text-center text-danger"><strong> {{ number_format( $sumrefund , 2 ) }}</strong></td>
                                </tr>
                        </tbody>
                    </table>
