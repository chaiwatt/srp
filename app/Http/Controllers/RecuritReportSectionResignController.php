...
class RecuritReportSectionResignController extends Controller
{
    public function Index(){
        ...
        return view('recurit.report.section.resign.index')->withProject($project)
    ...
    public function ExportPDF($month,$quater){
        ...
        $pdf->loadView("recurit.report.section.resign.pdfresign" , [ 'resign' => $resign , 'reason' => $reason, 'setting' => $setting, 'header' =>  $header ]);
    ...
    public function ExportExcel($month,$quater){
        ...
        $summary_array[] = array('รหัสตำแหน่ง','คำนำหน้า','ชื่อ','นามสกุล','ตำแหน่งที่สมัคร','วันที่ลาออก','เหตุผล'); 
        if(count($resign) > 0){
        ...
        $excelfile = Excel::create("resignreport", function($excel) use ($summary_array){
            $excel->setTitle("รายการลาออก");
            $excel->sheet('รายการลาออก', function($sheet) use ($summary_array){
