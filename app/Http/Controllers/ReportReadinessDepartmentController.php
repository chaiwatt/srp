...
class ReportReadinessDepartmentController extends Controller
{
    public function Index(){
        ...
        return view('report.readiness.department.index')->withSetting($setting)
    ...
    public function ExportPDF($month,$quater,$setyear){
        ...
        $pdf->loadView("report.readiness.department.pdfreadiness" , [ 
    ...
    public function ExportSinglePDF($id){
        ...
        $cost = ReadinessExpense::where('project_readiness_id' , $id)
                                ...
                                ->first(); 
        $header = "สำนักงาน" . $department->department_name;
        $pdf->loadView("report.readiness.department.singlepdf" , [ 'readiness' => $readiness , 'participate' => $participate, 'setting' => $setting, 'header' =>  $header , 'trainer' => $trainer , 'company' => $company ,'numofficer' => $numofficer, 'cost' => $cost]);
        return $pdf->download('readinessreport.pdf');     
        

    }      

    public function ReadinessChart(){
        if( $this->authdepartment() ){
            return redirect('logout');
        }
        $auth = Auth::user();
        ...
        return view('report.readiness.department.readinesschart')->withSetting($setting)
                                ->withReadiness($readiness)
                                ->withProjectreadiness($projectreadiness)
                                ->withParticipategroup($participategroup)
                                ->withReadinesssection($readinesssection)
